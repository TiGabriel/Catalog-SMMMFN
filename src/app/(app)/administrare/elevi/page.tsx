...
export default async function StudentsAdminPage({ searchParams }: { searchParams: Promise<{ clasa?: string }> }) {
  const sp = await searchParams;
  const actor = await getPageActor();
  const classes = await load(listClasses(actor));
  const selected = classes.find((c) => c.id === sp.clasa) ?? classes[0];
  const students = selected ? await load(listStudentsAdmin(actor, selected.id)) : [];
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        {classes.map((c) => (
          <Link key={c.id} href={`?clasa=${c.id}`} className={`rounded-full border px-3 py-1 text-sm ${c.id === selected?.id ? "border-primary bg-primary text-primary-contrast" : "border-border hover:bg-surface-2"}`}>
            {c.code}
          </Link>
        ))}
      </div>
      {selected && (
        <>
          ...
          <Card>
            <CardHeader title={`Clasa ${selected.code} – ${students.length} elevi`} />
            {students.length === 0 ? (
